...
class MyThread_KalmanBP(QThread):
    # 导入一个信号，用来接受读取的
    KalmanBP_signal = pyqtSignal(str, str, str, str, str, str, str)

    def __init__(self):
        super().__init__()

    def get_model(self, dates, prices, date_time, waterlevel1, waterlevel2, prewaterlevel1, prewaterlevel2):
        KalmanBP.main(eval(dates), eval(prices), date_time, waterlevel1, waterlevel2, eval(prewaterlevel1), eval(prewaterlevel2))
...
class Mywindow(QWidget):
    ...
    def but_load_clicked(self):
        ...
        print(self.prewaterlevel1)
        self.prewaterlevel2 = list(self.ws[int(self.line) + 1])[5:]
        print(self.prewaterlevel2)
    ...
    def but_pred_clicked(self):
        ...
        if self.selectItems_model == "EMD-ARIMA":
            ...
            self.myThread_KalmanBP.KalmanBP_signal.emit(str(self.dates), str(self.prices), self.date_time, self.waterlevel1, self.waterlevel2, str(self.prewaterlevel1), str(self.prewaterlevel2))
            self.textedit.setPlainText("模型训练完毕！")
