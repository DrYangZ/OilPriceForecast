...
class MyThread_getdata(QThread):
    ...
    def get_picture(self, dates, prices):
        # print(type(eval(dates)))
        # print(type(eval(prices)))
        ##设定一些基本参数
...
class MyThread_ARIMA(QThread):
    # 导入一个信号，用来接受读取的
    ARIMA_signal = pyqtSignal(str, str, str)

    def __init__(self):
        super().__init__()

    def get_model(self, dates, prices, date_time):
        LSTMARIMA.main(eval(dates), eval(prices), date_time)
...
class MyThread_KalmanBP(QThread):
    # 导入一个信号，用来接受读取的
    KalmanBP_signal = pyqtSignal(str, str, str)

    def __init__(self):
        super().__init__()

    def get_model(self, dates, prices, date_time):
        KalmanBP.main(eval(dates), eval(prices), date_time)
...
class MyThread_ARIMABF(QThread):
    # 导入一个信号，用来接受读取的
    ARIMABF_signal = pyqtSignal(str, str, str)

    def __init__(self):
        super().__init__()

    def get_model(self, dates, prices, date_time):
        ARIMARF.main(eval(dates), eval(prices), date_time)
...
class MyThread_EMDARIMA(QThread):
    # 导入一个信号，用来接受读取的
    EMDARIMA_signal = pyqtSignal(str, str, str)

    def __init__(self):
        super().__init__()

    def get_model(self, dates, prices, date_time):
        EMDARIMA.main(eval(dates), eval(prices), date_time)
...
class Mywindow(QWidget):
    ...
    def init_ui(self):
        ...
        # self.time_layout.addWidget(self.label5)
        # self.time_layout.addWidget(self.day_lineEdit)
    ...
    def but_load_clicked(self):
        ...
        self.dates = list(self.ws[0])[4:]
        self.base_information = list(self.ws[1])[:4]
        print(self.base_information)
        self.prices = list(self.ws[int(self.line) - 1])[4:]
        # time.sleep(10)
    ...
    def but_pred_clicked(self):
        ...
        if self.selectItems_model == "EMD-ARIMA":
            ...
            self.myThread_EMDARIMA.EMDARIMA_signal.emit(str(self.dates), str(self.prices), self.date_time)
            ...
            self.myThread_ARIMABF.ARIMABF_signal.emit(str(self.dates), str(self.prices), self.date_time)
            self.textedit.setPlainText("模型训练完毕！")
            ...
            self.myThread_KalmanBP.KalmanBP_signal.emit(str(self.dates), str(self.prices), self.date_time)
            self.textedit.setPlainText("模型训练完毕！")
            ...
            self.myThread_ARIMA.ARIMA_signal.emit(str(self.dates), str(self.prices), self.date_time)
            self.textedit.setPlainText("模型训练完毕！")
