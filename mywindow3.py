...
class MyThread_ARIMA(QThread):
    ...
    def get_model(self, dates, prices, date_time, waterlevel1, waterlevel2):
        LSTMARIMA.main(eval(dates), eval(prices), date_time, eval(waterlevel1), eval(waterlevel2))
...
class MyThread_ARIMABF(QThread):
    ...
    def get_model(self, dates, prices, date_time, waterlevel1, waterlevel2):
        ARIMARF.main(eval(dates), eval(prices), date_time, eval(waterlevel1), eval(waterlevel2))
...
class MyThread_EMDARIMA(QThread):
    ...
    def get_model(self, dates, prices, date_time, waterlevel1, waterlevel2):
        EMDARIMA.main(eval(dates), eval(prices), date_time, eval(waterlevel1), eval(waterlevel2))
...
class Mywindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.waterlevel1_lineEdit.setMaximumWidth(80)
        ...
        self.water2_lineEdit.setMaximumWidth(80)
        ...
        #航线距离
        self.Route_distance = QHBoxLayout()
        self.label_distance = QLabel(self)
        self.label_distance.setMaximumWidth(55)
        # self.label1.setFixedSize(50, 20)
        self.label_distance.setText("航线距离：")
        self.Route_distance.addWidget(self.label_distance)
        self.distance_lineEdit = QLineEdit()
        self.distance_lineEdit.setMaximumWidth(30)
        self.distance_lineEdit.setStyleSheet(
            "QLineEdit { background-color: white; color: #333; border-style: solid; border-width: 2px; border-color: #ccc; border-radius: 5px; padding: 3px; }")
        self.Route_distance.addWidget(self.distance_lineEdit)
        self.label_distance2 = QLabel(self)
        self.label_distance2.setMaximumWidth(15)
        self.label_distance2.setText("km")
        self.Route_distance.addWidget(self.label_distance2)
        self.v_layout1_waterlevel.addLayout(self.Route_distance)

        #是否过支流
        self.if_tributary = QHBoxLayout()
        self.combobox_tributary = QComboBox()
        self.combobox_tributary.setMaximumWidth(100)
        self.combobox_tributary.setStyleSheet(
            "QComboBox { background-color: white; color: #333; border-style: solid; border-width: 2px; border-color: #ccc; border-radius: 5px; padding: 3px; }"
            "QComboBox::drop-down { subcontrol-origin: padding; subcontrol-position: top right; width: 20px; border-left-width: 1px; border-left-color: #ccc; border-left-style: solid; background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #f6f7fa, stop:1 #dadbde); }"
            "QComboBox::down-arrow { image: url(down_arrow.png); }")
        # self.combobox.setFixedSize(150, 50)
        # 给下拉列表添加选项
        self.combobox_tributary.addItem("是否过支流")
        self.combobox_tributary.addItems(['是', '否'])
        self.if_tributary.addWidget(self.combobox_tributary)
        self.v_layout1_waterlevel.addLayout(self.if_tributary)
